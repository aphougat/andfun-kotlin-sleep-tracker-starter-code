...
class SleepQualityFragment : Fragment() {
    ...
    override fun onCreateView(inflater: LayoutInflater, container: ViewGroup?,
        ...
        viewModelFactory = SleepQualityViewModelFactory(SleepQualityFragmentArgs.fromBundle(arguments!!).sleepNightKey, SleepDatabase.getInstance(application)!!.sleepDatabaseDao )
        ...
        viewModel.sleepQualityComplete.observe(viewLifecycleOwner, Observer { status -> sleepQualityShared(status) })

        return binding.root
    }

    private fun sleepQualityShared(status : Boolean) {
        if(status) {
